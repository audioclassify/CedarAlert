...
The 'alert_email_process()' connects to an email server via TLS or SSL and attaches the image with caused the alert. The 'alert_email_process()' defaults to 3 attempts and will log failed attempts.

The 'alert_sms_process()' connects to an email server via TLS or SSL. The 'alert_sms_process()' defaults to 3 attempts and will log failed attempts. SMS messages are sent via the Verizon SMS portal (`YourCellNumber@vtext.com`), so you may need to change the 'alert_sms_process()' code if you have a different wireless carrier or wish to use a SMS service.
